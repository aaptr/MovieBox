...
export function ImageWithPreview({ src, alt, previewTitle }:
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <img src={`${imagePath}/w92${src}`} alt={alt} className="w-full h-full object-cover rounded-xl" />
      </DialogTrigger>
      <DialogContent className="w-screen bg-indigo-200 border-gray-600 dark:bg-gray-800 flex justify-center items-center">
        <DialogHeader>
          ...
          <DialogDescription className="flex justify-center items-center h-full w-full">
            <img src={`${imagePath}/original${src}`} alt={alt} className="max-h-full max-w-full object-contain rounded-xl" />
          </DialogDescription>
        </DialogHeader>
      </DialogContent>
    </Dialog>
